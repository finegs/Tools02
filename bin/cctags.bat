...
SET USE_HEADER=N
SET /P USE_HEADER=Use Header Files in Current Directory (Y/N)
IF ("%USE_HEADER%") == ("Y") (
	gcc -I%BOOST_INC% -I%BOOST_INC%/boost -Iinc -M * | sed -e 's/[\\ ]/\n/g' | sed -e '/^$/d' -e '/\.o:[ \t]*$/d' | ctags -L - --c++-kinds=+p --extra=+q --fields=+l --language-force=C++
) ELSE (
	ctags -R --c++-kinds=+p --extra=+q --fields=+l --language-force=C++ .
)
